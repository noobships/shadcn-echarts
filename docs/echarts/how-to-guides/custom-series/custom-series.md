...
The main development task is to implement a `renderItem`. For documentation, see the [Configuration Manual](https://echarts.apache.org/option.html#series-custom.renderItem).
...
If the custom series you develop is not intended for reuse, you can also implement the rendering algorithm for the custom series directly in `renderItem`. You can find many examples in the [Official Custom Series Examples](https://echarts.apache.org//examples#chart-type-custom) and develop based on them.